...
RSpec.describe Types::QueryType do
  describe 'display items' do
    it 'can query all items' do
      ...
      cats.items.create(name: 'Sean', quantity: 1, measurement: "oz", description: "this cat is the worst but I love him.", time_duration: "days", available: true, user_id: user.id, posting_id: posting.id)
      cats.items.create(name: 'Rhonda', quantity: 4, measurement: "lbs", time_duration: "months", available: true, user_id: user.id, posting_id: posting.id)
      lawn.items.create(name: 'Mower', quantity: 12.5, time_duration: 'hours', available: true, user_id: user.id, posting_id: posting.id)
      lawn.items.create(name: 'Fertilizer', quantity: 1, measurement: "oz", time_duration: 'days', available: true, user_id: user.id, posting_id: posting.id )
